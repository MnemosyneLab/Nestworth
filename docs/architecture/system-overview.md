...
TanStack Router defines explicit routes for startup, onboarding, overview, activity, investments, instruments, account list and detail, institutions, groups, general settings, and member settings. Primary feature pages load through route-level code splitting. Account owner, category, institution, and group filters, plus Activity list filters, live in URL search parameters so refresh and detail navigation preserve context.
...
- Logs identify lifecycle and failure events but must not include balances, notes, quantities, Account names, Instrument symbols, quote values, raw legs, ownership input, image bytes, database contents, or other sensitive payloads.
...
Later releases may add live providers, analytics, imports, or sync, but must preserve these boundaries:
...
- Origin and adjustment quantities remain unknown-basis until a later release records an explicit basis correction.
