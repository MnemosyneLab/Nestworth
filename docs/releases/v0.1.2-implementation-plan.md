...
**Status:** `Planned`. No phase is currently implemented.
...
- Open or close the market-data and FX provider decision using the Preparation Gate in the release contract, with an owner and a deadline before Phase 6.
- If selected now, record provider credential storage, market coverage, rate limits, license terms, and failure behavior.
- Capture a sanitized v0.1.1 database fixture containing Household, references, Accounts, Ownership, Account Values, archives, Settings, and Media references.
- Add the v0.1.2 golden valuation fixture and expected exact decimals.
- Record the current generated IPC surface and v0.1.1 Overview output as compatibility evidence.
- Record that any provider requiring a secret also requires an approved macOS Keychain boundary before its adapter is implemented.
...
- Add portfolio allocation read models for currency, country, and Instrument type.

**Required tests**

- Golden CNY/SGD/USD scenario totals exactly `62190 CNY`.
- Direct and inverse FX produce the same rounded base result.
- Identity conversion needs no FX quote.
...
**Entry requirement:** Phase 0 provider decision is approved.

**Deliverables**

- Implement QuoteProvider and FxProvider traits plus the selected adapter or adapters.
- Implement provider Instrument search and provider-identity attachment.
- Implement refresh for one Instrument, required FX, all prices, and everything.
...
- Implement approved credential storage without exposing secrets through IPC.
...
- Three Holdings of one Instrument trigger one provider quote request.
- Direct and inverse needs for one FX pair trigger one normalized request.
- A failed item does not erase the last quote or roll back successful items.
...
- Add `/investments` navigation and route-level lazy loading.
- Present current portfolio value, completeness coverage, positions, account totals, quote status, and refresh progress.
- Present allocation by native currency, country, and Instrument type.
...
- Partial refresh updates successful rows and keeps failed rows actionable.
...
- Re-audit transactions, retained archive references, query counts, decimal overflow, quote precedence, freshness clocks, and error shielding.
- Re-run v0.1.1 regression suites and all v0.1.2 golden cases.
- Test migration and blocked startup using isolated databases, including WAL/SHM snapshot behavior.
...
| --- | --- | --- |
| 0 | Decisions, baseline, and fixtures | `Planned` |
| 1 | Domain values and portfolio entities | `Planned` |
| 2 | Migration and repositories | `Planned` |
| 3 | Manual portfolio application and IPC | `Planned` |
| 4 | ValuationService and Overview integration | `Planned` |
| 5 | Manual portfolio UI | `Planned` |
| 6 | Provider adapters and batch refresh | `Planned` |
| 7 | Investments experience | `Planned` |
| 8 | Integrity and compatibility closeout | `Planned` |
| 9 | Release closeout | `Planned` |
