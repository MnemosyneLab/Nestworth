...
Phase 0 locks the native transport to direct `reqwest = 0.13.4` with default
features disabled and the `rustls` feature enabled. Rust owns the client,
URL construction, redirect policy, timeout, semaphore, and response-size
limits. The frontend keeps its existing IPC-only CSP and capability surface.
The dependency is included in the arm64 build validation before any production
Yahoo request is enabled.
