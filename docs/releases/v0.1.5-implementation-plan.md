...
**Status:** `Phase 10 implemented; Phase 11 planned`. The repository version is already `0.1.5`, but version synchronization is not feature evidence.
...
- Rust: search validation and SQLite smoke coverage pass; generated IPC drift, capabilities allowlist, `cargo fmt --all -- --check`, and `cargo clippy --all-targets -- -D warnings` pass. The repository-wide Rust suite reaches 487 passed and 6 pre-existing attribution failures under the current date environment; none touches Phase 10 files.
...
| 5 | Backup creation and inspection | `Implemented; full suite has unrelated baseline failures` |
