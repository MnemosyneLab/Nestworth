...
**Status:** `Phase 12 local release-candidate validation complete; public distribution closeout pending`. The repository version is already `0.1.5`, but version synchronization is not feature evidence.
...
- The Apple Silicon `aarch64-apple-darwin` application and DMG were built at version `0.1.5` with minimum macOS `26.0`. The exact DMG was verified, mounted read-only, launched with isolated application data, and checked for schema-5, SQLite-integrity, and foreign-key validity.
- The local artifact is ad hoc signed because no valid Developer ID signing identity is available. It is explicitly controlled-test only and is not notarized, stapled, or publicly distributable.
- Exact artifact details and the remaining device/release-owner gates are recorded in [v0.1.5 Release Evidence](v0.1.5-release-evidence.md).
...
| 12 | Release closeout | `Local validation complete; public closeout pending` |
