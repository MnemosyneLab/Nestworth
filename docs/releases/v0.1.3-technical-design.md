...
**Status:** `Planned`. The design is approved; names describe required responsibilities rather than current code. Current implemented truth remains schema migration `002` and the v0.1.2 architecture documents until this plan is implemented.
...
| FX rate | Present only for explicit cross-currency transfer legs |
...
| Transfer | Source, destination, exact source and destination amounts; same-currency equality or explicit cross-currency rate |
...
destination amount = source amount × fxRate
```

The destination amount must equal the checked full-precision conversion rounded once to Money scale. The stored Activity retains source amount, destination amount, and transaction FxRate. A fee is an additional explicit Fee leg or linked Fee Activity; it is never hidden in the transfer rate.
...
Market-price and FX movements are not Activities. They are differences between snapshot valuations after removing recorded Activity effects. v0.1.3 labels snapshot provenance but defers formal attribution to v0.1.4.
