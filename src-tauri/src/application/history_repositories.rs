...
pub async fn list_fx_preference_observations(
    tx: &mut Transaction<'_, Sqlite>,
    household_id: &str,
) -> Result<Vec<FxPreferenceObservationRecord>, AppError> {
    query_count::record("fx_preference_observations");
    sqlx::query(
        "SELECT id, household_id, currency_a, currency_b, source_kind, effective_at, created_at
         FROM fx_preference_observations
         WHERE household_id = ?
    ...
    .fetch_all(&mut **tx)
    .await
    .map_err(|error| map_read_error("history.fx_preferences_list_failed", error))?
    .into_iter()
    .map(|row| {
        Ok(FxPreferenceObservationRecord {
            id: required_text(&row, "id")?,
